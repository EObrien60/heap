...
function createStore({ dbFile, imagesDir }) {
  ...
  function updateSpace(id, patch) {
    ...
    if (typeof patch.name === 'string' && patch.name.trim()) {
      sp.name = patch.name.trim();
      // Re-seed hints from the new name unless the caller supplies explicit hints,
      // so the move-nudge keeps matching the space's current name after a rename.
      if (!Array.isArray(patch.hints)) sp.hints = nameToHints(sp.name);
    }
